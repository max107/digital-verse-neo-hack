...
namespace DvNft
{
    [DisplayName("DigitalVerse.NftContract")]
    [SupportedStandards("NEP-11")]
    [ContractPermission("*", "onNEP11Payment")]
    public class DvNftContract : SmartContract
    {
        ...
        public static UInt256 Mint(string name, string description, string image)
        {
            ...
            var tokenIdString = nameof(DvNftContract) + id;
            var tokenId = (UInt256)CryptoLib.Sha256(tokenIdString);

            var tokenState = new DvNftContract.TokenState
            {
                Owner = UInt160.Zero,
                Name = name,
        ...
        public static void OnNEP17Payment(UInt160 from, BigInteger amount, object data)
        {
            if (data != null)
            {
                ...
                var token = (DvNftContract.TokenState)StdLib.Deserialize(tokenData);
